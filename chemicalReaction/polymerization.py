...
def plotConcentration(concentrations, concentrationNumpy, imageCounter, t):
    ...
    plt.xticks(
        [1, 2, 3, 4, 6, 8, 10, 12, 14], ["Mon", "Di", "Tri", 4, 6, 8, 10, 12, 14]
    )
...
with alive_bar(manual=True) as bar:
    while concentrations[0] > endConcentrationToBeReached:
        ...
        # update progress bar
        simulationProgress = (
            -1 / (1 - endConcentrationToBeReached) * (concentrations[0] - 1)
        )
...
assert (
    abs(finalAmountOfSubstance - initialConcMon) < 1e-3
), "There has been a loss or gain of mass during the simulation"
...
with alive_bar(len(concentrationPerTimeStamp)) as bar:
    for i in range(len(concentrationPerTimeStamp)):
        plotConcentration(
            concentrationPerTimeStamp[i], y[i], i, timepointsToBeUsedForNumpy[i]
        )
        bar()
...
print(
    "Note: The simulation time has been shortened for demonstration purpose. Re-run the script again with endConcentrationToBeReached = 1e-4 in line 53"
)
