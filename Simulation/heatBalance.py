...
totalSize = (0.5, 1.0)  # in meters
discretization = (np.array(totalSize) / meshSize).astype(
    int
...
stoneCenterOfGravity = (
    coalTopPosition - int((stoneDiameter / 2) / meshSize),
    int(discretization[1] / 2),
)

# array asignment
objectAssignment = (
    np.ones((discretization[0], discretization[1])) * objectIndex["hotAir"]
)  # assign air as default

for i in range(discretization[0]):
    for j in range(discretization[1]):
        d = (
            (j - stoneCenterOfGravity[1]) ** 2 + (i - stoneCenterOfGravity[0]) ** 2
        ) ** 0.5
...
objectAssignment[
    coalTopPosition:coalBottomPosition, edges : int(totalSize[1] / meshSize) - edges
] = objectIndex["coal"]
...
for i in range(discretization[0]):
    for j in range(discretization[1]):
        if objectAssignment[i, j] == objectIndex["stone"]:
            if (
                objectAssignment[i + 1][j] != objectIndex["stone"]
                or objectAssignment[i - 1][j] != objectIndex["stone"]
                or objectAssignment[i][j + 1] != objectIndex["stone"]
                or objectAssignment[i][j - 1] != objectIndex["stone"]
            ):
                isBoundary[i][j] = 1
            if (
                objectAssignment[i + 1][j] == objectIndex["stone"]
                and objectAssignment[i - 1][j] == objectIndex["stone"]
                and objectAssignment[i][j + 1] == objectIndex["stone"]
                and objectAssignment[i][j - 1] == objectIndex["stone"]
            ):
                isWithinStone[i][j] = 1
        if objectAssignment[i, j] == objectIndex["coal"]:
            if (
                objectAssignment[min(i + 1, discretization[0] - 1)][j]
                == objectIndex["coal"]
                and objectAssignment[max(i - 1, 0)][j] == objectIndex["coal"]
                and objectAssignment[i][min(j + 1, discretization[1] - 1)]
                == objectIndex["coal"]
                and objectAssignment[i][max(j - 1, 0)] == objectIndex["coal"]
            ):
                isWithinCoal[i][j] = 1
        if isBoundary[i, j] == 1:
            if (
                objectAssignment[i + 1][j] == objectIndex["coal"]
                or objectAssignment[i - 1][j] == objectIndex["coal"]
                or objectAssignment[i][j + 1] == objectIndex["coal"]
                or objectAssignment[i][j - 1] == objectIndex["coal"]
            ):
                isBoundaryToCoal[i][j] = 1
            if (
                objectAssignment[i + 1][j] == objectIndex["hotAir"]
                or objectAssignment[i - 1][j] == objectIndex["hotAir"]
                or objectAssignment[i][j + 1] == objectIndex["hotAir"]
                or objectAssignment[i][j - 1] == objectIndex["hotAir"]
            ):
...
heightToTemperature = (
    lambda h: 800
    / (totalSize[0] - (soilSize[0] + coalSize[0]))
    * (h - (soilSize[0] + coalSize[0]))
    + 600
)

Tair = np.ones((discretization[0], discretization[1])) * 270
for i in range(discretization[0]):
    Tair[i, edges : int(totalSize[1] / meshSize) - edges] = heightToTemperature(
        IndexToHeight(i)
    )
...
cpDict = {
    objectIndex["stone"]: 1,
    objectIndex["coal"]: 1.02,
    objectIndex["soil"]: 0.8,
    objectIndex["hotAir"]: 1.005,
}  # in kJ kg-1 K-1
densityDict = {
    objectIndex["stone"]: 2.5e3,
    objectIndex["coal"]: 0.25e3,
    objectIndex["soil"]: 0.92e3,
    objectIndex["hotAir"]: 0.783,
}  # in kg cm-3
thermalConductivityDict = {
    objectIndex["stone"]: 2.0,
    objectIndex["hotAir"]: 0.02,
}  # in J s-1 m-1 K-1


def visualizeObjectAssignment(objectAssignment):
    colors = [(173, 156, 146), (203, 56, 23), (100, 78, 20), (249, 178, 75)]
    colorsNormalized = [[x / 256.0 for x in rgbCode] for rgbCode in colors]
    cmap = LinearSegmentedColormap.from_list(
        "colorMapObjectAssignment", colorsNormalized, N=4
    )
...
def visualizeArray(objectAssignment, testArray, filename):
    ...
    cmapBW = LinearSegmentedColormap.from_list(
        "colorMapIsBoundaryAssignment", blackAndWhiteRGB, N=3
    )
...
def visualizeTemperatureField(filename, time, temperatureArray):
    ...
    for i in range(discretization[0]):
        for j in range(discretization[1]):
            if objectAssignment[i, j] == objectIndex["hotAir"]:
                ...
                objectAssignment[i, j] == objectIndex["soil"]
                or objectAssignment[i, j] == objectIndex["coal"]
                or objectAssignment[i, j] == objectIndex["stone"]
            ):
    ...
    colorsNormalized = [[x / 256.0 for x in rgbCode] for rgbCode in colors]
    cmap = LinearSegmentedColormap.from_list(
        "colorMapObjectAssignment", colorsNormalized
    )
    im = plt.imshow(
        temperatureOutput - 273, cmap=cmap, vmin=0
...
def getHeatConductivity(object):
    ...
    """
    heatConductivity = {
        objectIndex["coal"]: 0.3,
        objectIndex["soil"]: 1,
        objectIndex["stone"]: 10,
    }
        ...
        if objectAssignment[i, j] == objectIndex[object]:
            if i + 1 < discretization[0]:
                if objectAssignment[i + 1, j] == objectIndex[object]:
                    conductionEnthalpy = (
                        A
                        * getHeatConductivity(objectIndex[object])
                        * (temperatureArray[i, j] - temperatureArray[i + 1, j])
                        / meshSize
                    )
            ...
            if j + 1 < discretization[1]:
                if objectAssignment[i, j + 1] == objectIndex[object]:
                    conductionEnthalpy = (
                        A
                        * getHeatConductivity(objectIndex[object])
                        * (temperatureArray[i, j] - temperatureArray[i, j + 1])
                        / meshSize
                    )
                    enthalpyRateArray[i, j] -= conductionEnthalpy
        ...
        for i in range(discretization[0]):
            for j in range(discretization[1]):
                ...
                if isBoundaryToAir[i, j] == 1:
                    # radiation
                    enthalpyRateArray[i, j] -= (
                        epsilon * sigma * A * ((temperatureArray[i, j]) ** 4 - Ta**4)
                    )
                    ...
                    enthalpyRateArray[i, j] -= (
                        getHeatTransferCoefficient(objectIndex["hotAir"])
                        * A
                        * (temperatureArray[i, j] - Tair[i, j])
                    )

                if isBoundaryToCoal[i, j] == 1:
                    # convection
                    convectionEnthalpy = (
                        getHeatTransferCoefficient(objectIndex["coal"])
                        * A
                        * (temperatureArray[i, j] - temperatureArray[i + 1, j])
                    )
...
print(
    "Note: The simulation time has been shortened for demonstration purpose. Re-run the script again with tend = 1000 in line 94"
)
