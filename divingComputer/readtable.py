...
def visualizeDivingProfile(time, depth, filename):
    ...
    ax.annotate(
        f"PG1 = {pressureGroupAfterFirstDive}", xy=(time[2], 0.25), color="blue"
    )
    ax.annotate(
        f"PG2 = {pressureGroupAfterSurfaceInterval}",
        xy=(time[4] - 3, 0.25),
        color="blue",
    )
    ax.annotate(f"PG3 = {PG3}", xy=(time[6] - 3, 0.25), color="blue")
    ax.annotate(
        f"BT = {dataForDiveComputerDepthTime[0][1]} min",
        xy=(time[1] + 1, depth[1] + 0.25),
        color="orange",
    )
    ax.annotate(
        f"ST = {dataForDiveComputerDepthTime[1][1]} min",
        xy=(time[4] / 2, depth[3] + 0.25),
        color="orange",
    )
    ax.annotate(
        f"BT = {int(maximumBottomTime)} min",
        xy=(time[5] + 1, depth[5] + 0.25),
        color="orange",
    )
...
def combineProfiles(depthProfile, timeProfile):
    ...
    for i in range(2, len(timeProfile)):
        if i % 2 == 0:
            dataForDiveComputerDepthTime.append(
                (depthProfile[i], (timeProfile[i] - timeProfile[i - 1]))
            )
...
findPressureGroup = (
    {}
)  # dict that contains the minutes in the excel file for each depth
...
for i, column_name in enumerate(
    dfFindPressureGroup
):  # iterates through depths, i.e. 10 m, 12 m, ...
    if (
        i == 0
    ):  # first columns contains the new pressure group that we are interested in
        pressureGroup = dfFindPressureGroup[column_name]
    else:  # other columns contain the depth at which we are
        key = float(
            f"{float(column_name):1.02f}"
        )  # round the value because it has many digits in the excel file
...
def getKeyForDepth(depth):
    if depth > keysInFindPressureGroup[0]:
        raise RuntimeError(
            f"please make sure that the depth does not exceed 42m. I got {depth}"
        )
...
assert getKeyForDepth(-30) >= 30.0, "key extraction failed"
...
assert (
    getPressureGroup(30.48, 17) == "L"
), "Mapping of bottom time to pressure group failed"
...
pressureGroupAfterSurfaceInterval = pd.read_excel(
    dataPath, sheet_name="get new pressure group"
)
mappingSurfaceTimeToNewPgroup = {}
listmappingSurfaceTimeToNewPgroup = []
indexToPressureGroup = {}
for i, oldPGroup in enumerate(
    pressureGroupAfterSurfaceInterval["pressure group from table 1 new pressure group"]
):
...
for i, column_name in enumerate(pressureGroupAfterSurfaceInterval):
    ...
    for j in range(len(pressureGroupAfterSurfaceInterval)):
        ...
        if not pd.isnull(pressureGroupAfterSurfaceInterval[column_name].iloc[j]):
            mappingSurfaceTimeToNewPgroup[indexToPressureGroup[j]][
                pressureGroupAfterSurfaceInterval[column_name].iloc[j]
            ] = column_name


def getPressureGroupAfterSurfaceInterval(
    oldPG, surfaceTime, mappingSurfaceTimeToNewPgroup
):
...
assert (
    getPressureGroupAfterSurfaceInterval(
        "L", getSurfaceTimeAsDatetimeObject(42), mappingSurfaceTimeToNewPgroup
    )
    == "F"
), "Mapping of initial pressure group to new pressure group after surface interval failed"
...
for i, depth2ndDive in enumerate(
    dfBottomTimeSecondDive["depth in m pressure group at the end of surface intervall"]
    ...
    for j in range(0, len(depthsForSecondDive)):
        if not pd.isnull(dfBottomTimeSecondDive[column_name].iloc[j * 2]):
            residualNitrogenTime[column_name][
                depthsForSecondDive[j]
            ] = dfBottomTimeSecondDive[column_name].iloc[j * 2]
        if not pd.isnull(dfBottomTimeSecondDive[column_name].iloc[j * 2 + 1]):
            maxBottomTime[column_name][depthsForSecondDive[j]] = dfBottomTimeSecondDive[
                column_name
            ].iloc[j * 2 + 1]
...
def getMaxBottomTimeAndMaximumNitrogenResidual(
    currentPressureGroup, desiredDivingDepth
):
...
assert getMaxBottomTimeAndMaximumNitrogenResidual("N", 25) == (
    3.0,
    22.0,
), "Extraction of maximum bottom time and residual nitrogen failed"
...
surfaceTimeDatetimeObject = getSurfaceTimeAsDatetimeObject(
    dataForDiveComputerDepthTime[1][1]
)
pressureGroupAfterSurfaceInterval = getPressureGroupAfterSurfaceInterval(
    pressureGroupAfterFirstDive,
    surfaceTimeDatetimeObject,
    mappingSurfaceTimeToNewPgroup,
)

maximumBottomTime, resN2Time = getMaxBottomTimeAndMaximumNitrogenResidual(
    pressureGroupAfterSurfaceInterval, depthSecondDive
)
